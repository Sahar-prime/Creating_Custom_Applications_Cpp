...
unsigned int simpleRand() 
{
    ...
    seed = (1103515245 * seed + 12345) & 0x7FFFFFFF;//2,147,483,647
    return seed;
}
...
/*Поянение к этой функции.
Он работает по простому принципу:
Сравнивает программа даты сначала по году: если год первой даты больше, чем у второй, они должны поменяться местами.
Если годы равны, сравнивает месяцы.
Если и месяцы равны, сравнивает дни.
*/
